function [cluster_centers, cluster_assignments] = lloyd_clustering_dice(data, K, NP, max_iterations)
    % Input:
    ...
    for iteration = 1:max_iterations
        ...
        for k = 1:K
            ...
            % Find the medoid (minimize intra-cluster Dice dissimilarity)
            min_cost = inf;
            for i = 1:size(cluster_points, 1)
                cost = 0;
                for j = 1:size(cluster_points, 1)
                    intersection = sum(cluster_points(i, :) & cluster_points(j, :));
                    sum_points = sum(cluster_points(i, :)) + sum(cluster_points(j, :));
                    cost = cost + (1 - (2 * intersection / sum_points));
                end
                if cost < min_cost
                    min_cost = cost;
