function evaluate_classifier_dice(test_features, test_labels, cluster_centers, K, NP)
    % Input:
    ...
    for i = 1:N
        distances = zeros(K, 1);
        for k = 1:K
            intersection = sum(test_features(i, :) & cluster_centers(k, :));
            sum_data_cluster = sum(test_features(i, :)) + sum(cluster_centers(k, :));
